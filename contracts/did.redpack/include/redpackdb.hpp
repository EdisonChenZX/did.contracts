...
static const std::set<eosio::name> banks = { "amax.token"_n, "amax.mtoken"_n, "usdt.token"_n, "gsefundtoken"_n,
                                             "airc.token"_n, "tyche.token"_n, "cnyg.token"_n, "mdao.token"_n };
...
#define TG_TBL_NAME(name) [[eosio::table(name), eosio::contract("did.redpack")]]
   ...
   uint128_t get_unionid(const name &rec, uint64_t packid) { return ((uint128_t)rec.value << 64) | packid; }
   ...
   struct TG_TBL redpack_t {
      ...
      EOSLIB_SERIALIZE(
            redpack_t,
            (code)(sender)(pw_hash)(total_quantity)(receiver_count)(remain_quantity)(remain_count)(fee)(status)(type)(created_at)(updated_at))
   };
   ...
   struct TG_TBL claim_t {
      ...
      typedef eosio::multi_index<"claims"_n, claim_t, indexed_by<"unionid"_n, const_mem_fun<claim_t, uint128_t, &claim_t::by_unionid>>
